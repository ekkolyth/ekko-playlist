...
func (h *AuthHandler) UpdateUserProfile(w http.ResponseWriter, r *http.Request) {
	...
	emailVerificationRequired := os.Getenv("EMAIL_VERIFICATION") == "true"
	if emailVerificationRequired {
		// Get current user to check if email is changing
		currentUser, err := h.dbService.Queries.GetUserByID(ctx, userID)
		if err == nil && currentUser != nil {
			emailChanged := currentUser.Email != req.Email
			if emailChanged {
				// For email changes, Better Auth OTP verification should have been completed
				// on the frontend before calling this endpoint. We allow the update here
				// since verification was handled by Better Auth.
				// The user's emailVerified status will be updated by Better Auth after verification.
			}
		}
	}
