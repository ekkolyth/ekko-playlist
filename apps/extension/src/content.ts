...
// Inline YouTube URL parser to avoid CommonJS module issues
...
async function scrollToLoadContent(maxScrolls: number = 100): Promise<void> {
  ...
  for (let scrolls = 0; scrolls < maxScrolls; scrolls++) {
    ...
    await new Promise(resolve => setTimeout(resolve, clickedLoadMore ? 2000 : 1500));
    
    const currentItemCount = countItems();
    console.log(`Scroll ${scrolls + 1}: Found ${currentItemCount} items (was ${lastItemCount})`);
    ...
    if (currentItemCount > lastItemCount) {
      ...
      if (stableCount >= 3) {
        ...
        console.log(`Content stabilized after ${scrolls + 1} scrolls with ${currentItemCount} items`);
        return;
      }
    }
  }
  
  console.log(`Reached max scrolls (${maxScrolls}) with ${lastItemCount} items`);
}
...
async function extractVideoInfo(): Promise<VideoInfo[]> {
  ...
  for (const selector of playlistSelectors) {
    ...
    if (playlistItems.length > 0) {
      console.log(`Found ${playlistItems.length} playlist items using selector: ${selector}`);
      break;
    }
  }
  ...
  if (playlistItems && playlistItems.length > 0) {
    ...
    playlistItems.forEach((item, index) => {
      ...
      let linkElement = item.querySelector('a[href*="/watch?v="]') as HTMLAnchorElement;
      ...
      if (linkElement && linkElement.href) {
        ...
        if (parsedUrl.isValid && parsedUrl.videoId && parsedUrl.normalizedUrl) {
          if (!processedLinks.has(parsedUrl.videoId)) {
            ...
            if (videoTitle === 'Unknown Title' && linkElement.textContent) {
            ...
            if (channelElement) {
              ...
              if (!channelName || channelName === '') {
                channelName = channelElement.getAttribute('title')?.trim() ||
            ...
            console.log(`Skipping duplicate video ID: ${parsedUrl.videoId}`);
          }
        } else {
    ...
    console.log(`Processed ${videos.length} unique videos from playlist items`);
  }

  // Fallback: if no playlist items found, try to extract from general video links
  // But only if we're on a playlist page
  if (videos.length === 0 && window.location.href.includes('/playlist?list=')) {
      ...
      if (anchor.href && anchor.href.includes('/watch?v=')) {
        ...
        if (parsedUrl.isValid && parsedUrl.videoId && parsedUrl.normalizedUrl) {
          if (!processedLinks.has(parsedUrl.videoId)) {
            ...
            const container = anchor.closest('ytd-playlist-video-renderer, ytd-video-renderer, ytd-grid-video-renderer, ytd-playlist-video-list-renderer');
  ...
  const allPlaylistItems = document.querySelectorAll('ytd-playlist-video-renderer');
  console.log(`Total playlist items found in DOM: ${allPlaylistItems.length}`);
  console.log(`Unique videos extracted: ${videos.length}`);
  
  if (allPlaylistItems.length > videos.length) {
    console.warn(`Warning: Found ${allPlaylistItems.length} playlist items but only extracted ${videos.length} videos. Some items may have been skipped.`);
  }
