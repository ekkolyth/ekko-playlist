    ...
    handlers: {
      POST: async ({ request }) => {
        try {
          ...
          console.log('Verifying one-time token:', token.substring(0, 10) + '...');
          ...
          try {
            ...
            console.log('Session structure:', {
              ...
              userKeys: result?.session?.user ? Object.keys(result.session.user) : [],
            });
          } catch (error) {
          ...
          const user = result?.session?.user || result?.user || result?.session?.data?.user;
          ...
          if (!result || !result.session || !user) {
            console.log('Token verification failed - no session or user:', {
              ...
              hasSession: !!result?.session,
              hasUser: !!user,
              sessionStructure: result?.session ? Object.keys(result.session) : [],
            });
